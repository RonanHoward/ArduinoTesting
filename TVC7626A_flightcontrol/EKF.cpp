...
EKF::EKF() {
  ...
  aax = aay = aaz = agx = agy = agz = 0.1f;

  axPrev = ayPrev = azPrev = gxPrev = gyPrev = gzPrev = 0.0f;
...
void EKF::predict(float T) {
  // TODO: apply low-pass filter
  float p = fgx * DEG_TO_RAD;
  float q = fgy * DEG_TO_RAD;
  float r = fgz * DEG_TO_RAD;
...
void EKF::update() {
  ...
  phi_rad   = phi_rad   + K[0] * (fax - h[0]) + K[1] * (fay - h[1]) + K[2] * (faz - h[2]);
  theta_rad = theta_rad + K[3] * (fax - h[0]) + K[4] * (fay - h[1]) + K[5] * (faz - h[2]);
}

void EKF::lpf_acc(float ax, float ay, float az) {
  fax = aax * axPrev + (1-aax) * ax;
  fay = aay * ayPrev + (1-aay) * ay;
  faz = aaz * azPrev + (1-aaz) * az;
  axPrev = fax;
  ayPrev = fay;
  azPrev = faz;
}
void EKF::lpf_gyro(float gx, float gy, float gz) {
  fgx = agx * gxPrev + (1-agx) * gx;
  fgy = agy * gyPrev + (1-agy) * gy;
  fgz = agz * gzPrev + (1-agz) * gz;
  gxPrev = fgx;
  gyPrev = fgy;
  gzPrev = fgz;
}
