...
void loop() {
  ...
  if (startTime >= TELEMETRY_LOG_DELAY_MS) {
    if (startTime >= TELEMETRY_LOG_DELAY_MS + TELEMETRY_LOG_TIME_SEC*1000 + 100) {
      ...
      Serial.print(',');
      Serial.print(telemetry.logs[print_log_counter][4]);
      Serial.print(',');
      Serial.print(telemetry.logs[print_log_counter][5]);
      Serial.print(',');
      Serial.print(telemetry.logs[print_log_counter][6]);
      Serial.print(',');
      Serial.print(telemetry.logs[print_log_counter][7]);
      Serial.print(',');
      Serial.println(telemetry.logs[print_log_counter][8]);
      print_log_counter++;
