...
const int SERVO_UPDATE_INTERVAL = 50; // in ms
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (command.startsWith("x")) {
      ...
      if (animation.equals("xline")) {
        ...
        Serial.println("Playing animation \"xline\".");
      } else if (animation.equals("yline")) {
        current_animation = "yline";
        playing_animation = true;
        Serial.println("Playing animation \"yline\".");
      } else if (animation.equals("circle")) {
        current_animation = "circle";
        playing_animation = true;
        Serial.println("Playing animation \"circle\".");
      } else {
  ...
  if (playing_animation && (millis() - last_animation_frame >= SERVO_UPDATE_INTERVAL)) {
    ...
    else if (current_animation.equals("yline")) {
      gimbal.write(0,sin(now)*6.0); // radius = 6
    }

    else if (current_animation.equals("circle")) {
      gimbal.write(cos(now)*6.0,sin(now)*6.0);
    }
